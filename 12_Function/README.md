...
> 자바스크립트 엔진은 `코드의 문맥`에 따라 동일한 함수를 `함수 표현식` 또는 `함수 선언문`으로 해석하는 경우가 있다.

- `{ }(중괄호)` 는 `코드 블록` 일 수도 있고, `객체 리터럴` 일 수도 있다. → `{ }(중괄호)는 중의적 표현이다`
  - `{ }` 이 `단독`으로 존재한다? => 자바스크립트 엔진은 `{ } 을 블록문으로 해석`
  - `{ }` 이 `값으로 평가`되어야 할 문맥에서 `피연산자` 로 사용될 경우? => 자바스크립트 엔진은 `{ }(중괄호)를 객체 리터럴로 해석`
- 함수도 이와 같다.
  - `함수 리터럴` 이 단독으로 사용된다면? => `함수 선언문`으로 해석
  - `함수 리터럴` 이 값으로 평가되어야 하는(변수에 할당 or 피연산자로..) 문맥에 있다면?? => `함수 리터럴 표현식` 으로 해석

# 함수 선언문과 함수 리터럴 표현식

> 이 둘은 함수가 생성되는 것은 동일하다, 다만 `호출에서 차이가 있다.`
```jsx
// 함수 선언문으로 호출될 때
function foo() {
  console.log("foo"); // foo
...
```jsx
★★★ 결론적으로, 자바스크립트 엔진은 "함수 선언문"을 "함수 표현식으로 변환" → "함수 객체를 생성한다"
```
...
- 함수는 매개변수(parameter)의 개수와 인수(argument)의 개수가 일치하지 않아도 된다.
...
function mul(x, y) { // 파라미터 x,y 2개가 있다.
  console.log(x, y); // 1 undefined
}
mul(1); // 인수 1만 있다 => x만 참조.
...
sub(3, 2, 1); // 1, 파라미터 z가 없으므로, 인수 1은 자동적으로 arguments에 보관된다.
```
...
2️. 자바스크립트는 "동적 타입 언어"다. 따라서 자바스크립트 함수는 매개변수의 "타입을 사전에 지정할 수 없다."
```
...
- 따라서, 자바스크립트의 경우 `함수를 정의할 때, 인수가 전달되었는지 확인할 필요가 있다.`
  ...
  4. `정적 타입 선언` 이 가능한 `Typescript`를 사용하는 방법

```jsx
// 1️. typeof 연산자로 arguments 문제 방지
function add(x, y) {
...
console.log(add(2)); // TypeError: 인수는 모두 숫자(number)값 이어야 합니다. => y가 undefined로 참조되어 TypeError 발생
console.log(add("a", "b")); // TypeError: 인수는 모두 숫자(number)값 이어야 합니다. => string 값으로 참조되어 TypeError 발생

// 2️. "단축 평가"로 arguments 문제 방지
function mul(a, b, c) {
  a = a || 1;
  b = b || 1;
  c = c || 1;
...
// 3️. parameter default value 설정으로 argument 문제 방지
...
- 즉, 객체 argument 는 `참조 값(주소 값)이 복사되어 매개변수에 전달`
...
- `즉시 실행 함수(IIFE, Immediately Invoked Function Expression)`
  ...
  - 즉시 실행 함수는 `반드시 그룹 연산자( ... )로 감싸야 한다.` `(스프레드와 다름!)`
  ...
  ```
- `재귀 함수(recursive function)` => 재귀함수 파트에서 다루자!
