...
const person = new Person("Cha");
console.log(person); // Person { name: 'Cha', getPersonName: [Function (anonymous)] }
```
...
const person = new Person("Cha");
console.log(person); // {} => 리턴된 다른 객체가 튀어나온다
...
function Person(name) {
  ...
  return 10000; // 윈시값 Number가 리턴 되면?
}
const person = new Person("Cha");
console.log(person); // Person { name: 'Cha', getPersonName: [Function (anonymous)] }
//원시값은 무시되고 암묵적으로 this가 반환되는 것을 확인
```
...
// 일반 함수가 `new 연산자`와 함께, `생성자 함수`로 호출
const person = new Person("Cha");
console.log(person); // Person { name: 'Cha', getPersonName: [Function (anonymous)] }
```

> 반대로 `new 연산자 없이` , 생성자 함수를 호출하면 `일반 함수로 동작`
- 즉, 함수 객체 내부 메서드 중 `[[ Call ]]` 이 호출된다는 것
...
console.log(person.getPersonName()); // Hi, My Name is Cha
```
